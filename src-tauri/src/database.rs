...
impl Database {
    ...
    fn init_tables(&self) -> Result<()> {
        ...
        let _ = self
            .conn
            .execute("ALTER TABLE live_rooms ADD COLUMN short_id TEXT", []);
        let _ = self.conn.execute(
            "ALTER TABLE live_rooms ADD COLUMN uid INTEGER DEFAULT 0",
            [],
        );
        self.conn.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_live_rooms_platform_room ON live_rooms(platform, room_id)",
            [],
        )?;
        ...
        self.conn.execute(
            "CREATE TABLE IF NOT EXISTS record_tasks (
                ...
                requested_qn INTEGER DEFAULT 10000,
                actual_qn INTEGER DEFAULT 0,
                stream_type TEXT DEFAULT '',
        ...
        let _ = self.conn.execute(
            "ALTER TABLE record_tasks ADD COLUMN requested_qn INTEGER DEFAULT 10000",
            [],
        );
        let _ = self.conn.execute(
            "ALTER TABLE record_tasks ADD COLUMN actual_qn INTEGER DEFAULT 0",
            [],
        );
        let _ = self.conn.execute(
            "ALTER TABLE record_tasks ADD COLUMN stream_type TEXT DEFAULT ''",
            [],
        );
    ...
    pub fn get_all_rooms(&self) -> Result<Vec<LiveRoom>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, platform, room_id, short_id, uid, anchor_name, room_title, cover_url, avatar_url, is_live, created_at, auto_record_enabled, auto_record_daily_time, auto_record_until, last_schedule_trigger_date FROM live_rooms"
        )?;
        ...
        let rooms = stmt
            .query_map([], |row| {
                Ok(LiveRoom {
                    ...
                    room_title: row.get(6)?,
                    cover_url: row.get(7)?,
                    avatar_url: row.get(8)?,
                    is_live: row.get(9)?,
                    created_at: row.get(10)?,
                    auto_record_enabled: row.get(11)?,
                    auto_record_daily_time: row.get(12)?,
                    auto_record_until: row.get(13)?,
                    last_schedule_trigger_date: row.get(14)?,
                })
            })?
    ...
    pub fn get_room(&self, id: i64) -> Result<LiveRoom> {
        self.conn.query_row(
            "SELECT id, platform, room_id, short_id, uid, anchor_name, room_title, cover_url, avatar_url, is_live, created_at, auto_record_enabled, auto_record_daily_time, auto_record_until, last_schedule_trigger_date FROM live_rooms WHERE id = ?1",
            [id],
            |row| {
                Ok(LiveRoom {
                    id: row.get(0)?,
    ...
    pub fn add_room_full(
        ...
        self.conn.execute(
            "INSERT INTO live_rooms (platform, room_id, short_id, uid, anchor_name, room_title, cover_url, avatar_url, is_live) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)",
            rusqlite::params![platform, room_id, short_id, uid, anchor_name, room_title, cover_url, avatar_url, is_live],
        )?;
        Ok(self.conn.last_insert_rowid())
    }

    pub fn find_room_by_platform_id(
        &self,
        platform: &str,
        room_id: &str,
    ) -> Result<Option<LiveRoom>> {
        match self.conn.query_row(
            "SELECT id, platform, room_id, short_id, uid, anchor_name, room_title, cover_url, avatar_url, is_live, created_at, auto_record_enabled, auto_record_daily_time, auto_record_until, last_schedule_trigger_date FROM live_rooms WHERE platform = ?1 AND room_id = ?2",
            rusqlite::params![platform, room_id],
            |row| {
                Ok(LiveRoom {
                    id: row.get(0)?, platform: row.get(1)?, room_id: row.get(2)?,
                    short_id: row.get(3)?, uid: row.get(4)?, anchor_name: row.get(5)?,
                    room_title: row.get(6)?, cover_url: row.get(7)?, avatar_url: row.get(8)?,
                    is_live: row.get(9)?, created_at: row.get(10)?, auto_record_enabled: row.get(11)?,
                    auto_record_daily_time: row.get(12)?, auto_record_until: row.get(13)?,
                    last_schedule_trigger_date: row.get(14)?,
                })
            },
        ) {
    ...
    pub fn update_room_live_status(
        ...
        self.conn.execute(
            "UPDATE live_rooms SET short_id = COALESCE(?1, short_id), uid = CASE WHEN ?2 > 0 THEN ?2 ELSE uid END, anchor_name = ?3, room_title = ?4, cover_url = ?5, avatar_url = ?6, is_live = ?7 WHERE id = ?8",
            rusqlite::params![short_id, uid, anchor_name, room_title, cover_url, avatar_url, is_live, id],
        )?;
        Ok(())
    }
    ...
    pub fn has_running_tasks_for_room(&self, room_id: i64) -> Result<bool> {
        let count: i64 = self.conn.query_row(
            "SELECT COUNT(*) FROM record_tasks WHERE room_id = ?1 AND status IN ('preparing', 'recording', 'finalizing')",
            [room_id],
            |row| row.get(0),
        )?;
        Ok(count > 0)
    ...
    pub fn get_all_tasks(&self) -> Result<Vec<RecordTask>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, room_id, status, start_time, end_time, file_path, file_size, trigger, requested_qn, actual_qn, stream_type FROM record_tasks ORDER BY id DESC"
        )?;
        ...
        let tasks = stmt
            .query_map([], |row| {
                Ok(RecordTask {
                    ...
                    file_size: row.get(6)?,
                    trigger: row.get(7)?,
                    requested_qn: row.get(8)?,
                    actual_qn: row.get(9)?,
                    stream_type: row.get(10)?,
                })
            })?
            .collect::<Result<Vec<_>>>()?;
    ...
    pub fn get_task(&self, id: i64) -> Result<RecordTask> {
        self.conn.query_row(
            "SELECT id, room_id, status, start_time, end_time, file_path, file_size, trigger, requested_qn, actual_qn, stream_type FROM record_tasks WHERE id = ?1",
            [id],
            |row| {
                Ok(RecordTask {
                    id: row.get(0)?,
    ...
    pub fn add_task(
        ...
        self.conn.execute(
            "INSERT INTO record_tasks (room_id, status, trigger, requested_qn, actual_qn, stream_type) VALUES (?1, 'preparing', ?2, ?3, ?4, ?5)",
            rusqlite::params![room_id, trigger, requested_qn, actual_qn, stream_type],
        )?;
        Ok(self.conn.last_insert_rowid())
    }
    ...
    pub fn mark_task_finalizing(&self, id: i64, file_size: i64) -> Result<()> {
        self.conn.execute(
            "UPDATE record_tasks SET status = 'finalizing', end_time = datetime('now'), file_size = ?1 WHERE id = ?2 AND status IN ('preparing', 'recording', 'finalizing')",
    ...
    pub fn reconcile_incomplete_tasks(&self) -> Result<()> {
        let stale_tasks = {
            let mut stmt = self.conn.prepare(
                "SELECT id, file_path FROM record_tasks WHERE status IN ('preparing', 'recording', 'finalizing')",
            )?;
            let tasks = stmt
                .query_map([], |row| {
...
mod tests {
    ...
    fn reconciles_stale_recordings_from_file_state() {
        ...
        let interrupted_id = db
            .add_task(room_id, "manual", 10000, 250, "FLV")
        ...
        let failed_id = db
            .add_task(room_id, "auto", 10000, 250, "FLV")
            .expect("add failed task");
        db.update_task_status_and_path(failed_id, "finalizing", None)
        ...
        assert_eq!(interrupted.actual_qn, 250);
...

        {
            ...
            connection
                .execute_batch(
                    ...
                    VALUES ('bilibili', 'legacy-room', 'legacy-anchor');
