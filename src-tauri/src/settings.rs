...
pub struct AppSettings {
    ...
    pub preserve_source_after_convert: bool,
...
impl Default for AppSettings {
    fn default() -> Self {
        ...
        AppSettings {
            ...
            preserve_source_after_convert: true,
...
pub fn default_db_dir() -> PathBuf {
    let home = std::env::var("USERPROFILE")
        .or_else(|_| std::env::var("HOME"))
        .expect("无法获取用户目录");
    PathBuf::from(&home).join(".bilibili-recorder")
}

pub fn default_db_path() -> PathBuf {
    default_db_dir().join("bilibili_recorder.db")
}
...
pub fn save_settings(settings: &AppSettings) -> Result<(), String> {
    ...
    let path = settings_path();
    std::fs::create_dir_all(default_db_dir()).map_err(|e| format!("创建配置目录失败: {}", e))?;
    let json =
...
pub fn migrate_db(new_path: &str) -> Result<String, String> {
    ...
    if std::fs::canonicalize(&old_path).ok() == std::fs::canonicalize(&new_path).ok()
        && new_path.exists()
    {
