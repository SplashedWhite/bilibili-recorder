...
房间监控 · 手动与自动录制 · 每日定时 · FLV 转 MP4 · 多画质
...
- 设置每天固定时间开启一个自动监控窗口
- 原画、蓝光、超清、高清、流畅画质；不可用时接受官方实际降级
- 记录请求画质、实际画质和流类型
- FFmpeg 多 CDN 启动探测、有限重连和异常结束识别
- 手动或自动将 FLV 无损封装为 MP4，并可选择是否保留 FLV
...
| 录制目录 | FLV 和 MP4 的保存位置 | `~/BilibiliRecordings` |
| 自动转 MP4 | 录制正常完成后自动转换 | 关闭 |
| 保留原始 FLV | MP4 成功后是否保留源文件 | 开启 |
...
- MP4 转换先写临时文件并验证成功，再完成改名和可选的 FLV 删除。
