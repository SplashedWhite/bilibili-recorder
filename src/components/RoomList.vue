    ...
    <div class="add-bar">
      <div class="add-input-wrap">
        ...
        <input
          v-model="newUrl"
          class="add-input"
          placeholder="粘贴 Bilibili 直播间链接或房间号"
        ...
        <div v-for="room in rooms" :key="room.id" class="room-card">
          ...
          <div class="room-actions">
            <el-button
              ...
              :loading="isPreparing(room.id)"
              :disabled="isPreparing(room.id)"
            ...
            >
              ...
              {{ isPreparing(room.id) ? '准备中' : isRecording(room.id) ? '停止' : '录制' }}
            </el-button>
            <button
              class="icon-btn"
...
function getActiveTask(roomId: number) {
  return tasks.value.find(t => t.room_id === roomId && ['preparing', 'recording'].includes(t.status))
}

function isRecording(roomId: number) {
  return getActiveTask(roomId)?.status === 'recording'
}

function isPreparing(roomId: number) {
  return getActiveTask(roomId)?.status === 'preparing'
}

function getAutoRecordText(room: LiveRoom) {
  if (!room.auto_record_enabled) return '自动录制'
  if (getActiveTask(room.id)) return '自动录制已开启'
...
async function toggleRecord(room: LiveRoom) {
  ...
  if (activeTask) {
    if (activeTask.status === 'preparing') return
    try {
